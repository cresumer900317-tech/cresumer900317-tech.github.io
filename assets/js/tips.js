document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    function renderList(list) {
      ...
      return list.map(t => `
        <div class="board-row" onclick="openTip(${t.id})" style="cursor:pointer;">
    ...
    document.querySelector("main").innerHTML = `
      <div class="page-card">
        <div class="container">
          <div style="padding:28px 0 20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
            ...
            <button onclick="showWrite()" class="board-write-btn">✏️ 꿀팁 등록</button>
          </div>

          <!-- 카테고리 필터 -->
          <div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap;">
            ${["전체","공략","성장","캐릭터","아이템","기타"].map(c => `
              <button class="cat-btn${c==="전체"?" active":""}" data-cat="${c}" onclick="filterAndRender('${c}')">${c}</button>
            `).join("")}
          </div>
          ...
          <div id="writeForm" class="board-write-form" style="display:none;">
            ...
            <div style="display:flex;gap:8px;margin-top:10px;justify-content:flex-end;">
              ...
              <button onclick="submitTip()" class="board-submit-btn">등록</button>
            </div>
          </div>
      ...
      <!-- 모달 -->
      <div id="tipModal" class="board-modal-bg" style="display:none;" onclick="if(event.target===this)closeTipModal()">
    ...
    document.getElementById("tipContent").innerHTML = `
      <div style="padding:28px 28px 24px;">
        ...
        <div style="margin-top:20px;display:flex;align-items:center;gap:8px;">
          <button onclick="likeTip(${tip.id})" style="padding:7px 16px;background:#fff;border:1px solid #fca5a5;border-radius:999px;color:#ef4444;font-weight:700;font-size:0.85rem;cursor:pointer;">
            ❤️ ${tip.likes||0}
          </button>
        </div>
        <div style="margin-top:16px;display:flex;justify-content:space-between;">
          <button onclick="closeTipModal()" class="board-cancel-btn">← 목록</button>
          ${(isOwner || isAdmin) ? `<button onclick="deleteTip(${tip.id})" class="board-delete-btn">🗑️ 삭제</button>` : ""}
        </div>
      </div>
    `;
